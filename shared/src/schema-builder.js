...
export function buildSchema(title, description, credentialType, version, attributes) {
    try {
        ...
        const [attributesBlockWithSaid, attributesSaidObject] = Saider.saidify(attributesBlock, MtrDex.Blake3_256, Serials.JSON, "$id");
...
export async function calculateSchemaSaid(metadata) {
    ...
    let saidString;
    try {
        const [schemaWithSaid, saidObject] = Saider.saidify(schema, MtrDex.Blake3_256, Serials.JSON, "$id");
    ...
    const [_, attributesSaidObject] = Saider.saidify(attributesBlock, MtrDex.Blake3_256, Serials.JSON, "$id");
    const attributesSaid = attributesSaidObject?.$id
        ? String(attributesSaidObject.$id)
        : "";
    return {
